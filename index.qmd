...
Use the `tidyr` function `expand.grid` to create a data frame with columns $x$ and $y$, each containing values of $x$. The resulting data frame should have 49 rows.
...
Get all rows in your data frame where $x$ and $y$ have no common factors (that is, LCM(x, y) = x*y). Store these rows in a data frame called `no_common_factors`.
...
In R and python, write a function, `gcd(x, y)`, which will find the greatest common denominator between $x$ and $y$.
...
For each $x$, create a data frame with the $y$ that has the largest common denominator. 
